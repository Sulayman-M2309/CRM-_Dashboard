...
export const HandleAllDepartments = async (req, res) => {
  try {
    const departments = await Department.find({
      ...
      "employees notice HumanResources",
      "firstname lastname email contactnumber title audience createdby"
    );
    ...
    if (employeeIDArray) {
      ...
      if (RejectedEmployees.length > 0) {
        return res.status(400).json({
          success: false,
          message: `Some Employees Are Already Belongs To ${SelectedDepartment.name} Department`,
      ...
      return res.status(200).json({
        success: true,
        message: `Employees Added Successfully to ${SelectedDepartment.name} Department`,
...
export const HandleDeleteDepartment = async (req, res) => {
  try {
    ...
    if (action === "delete-employee") {
      ...
      for (let index = 0; index < employeeIDArray.length; index++) {
        department.employees.splice(
          department.employees.indexOf(employeeIDArray[index]),
          1
        );
      }
