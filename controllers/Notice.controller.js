    ...
    if (audience === "Department-Specific") {
      ...
      if (checknotice) {
        return res
          ...
          .json({
            success: false,
            message: "Specific Notice Record Already Exists",
          });
      }
      ...
      return res
        ...
        .json({
          success: true,
          message: "Specific Notice Created Successfully",
          data: notice,
        });
    }
...
export const HandleAllNotice = async (req, res) => {
  try {
    const notices = await Notice.find({ organizationID: req.ORGID }).populate(
...
export const HandleDeleteNotice = async (req, res) => {
  try {
    ...
    if (!notice) {
      return res
        .status(404)
        .json({ success: false, message: "Notice Record Not Found" });
    }
